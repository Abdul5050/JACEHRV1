...
<div id="wrapper">
	...
	<h2>Add User</h2>
	 ...
	 <form action="" method="POST">
        ...
        <label>
          <input class="btn btn-success btn-lg btn-flat btn-rect"type='submit' name='submit' value='Add User'>
        </label>
      </form>
      <?php
     if(isset($_POST['submit'])){
		...
		if(!isset($error)){
			...
			try {
				...
				$stmt = $db->prepare('INSERT INTO users (username,password,email,name) VALUES (:username, :password, :email, :name)') ;
				$stmt->execute(array(
					':username' => $username,
