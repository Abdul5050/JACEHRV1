...
if(isset($_GET['deluser'])){ 
	...
	if($_GET['deluser'] !='1'){
		...
		$stmt = $db1->prepare('DELETE FROM users WHERE id = :id') ;
...
<div id="wrapper">
	...
	<?php
		try {
			...
			while($row = $stmt->fetch()){
				...
				<td>
					<a href="edit-user.php?id=<?php echo $row['id'];?>"class="btn btn-primary"><i class="icon-pencil icon-white"></i> Edit</a> 
					
						<a href="javascript:deluser('<?php echo $row['id'];?>','<?php echo $row['username'];?>')" class="btn btn-danger"><i class="icon-remove icon-white"></i>Delete</a>
	...
	<p><a href='add-user.php'class="btn btn-success"><i class="icon-plus-sign icon-white"></i> Add User</a></p>
